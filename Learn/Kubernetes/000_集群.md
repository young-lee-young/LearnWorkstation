...
sudo docker ps -a | grep kube-apiserver | grep Exited | awk '{print $1}' | xargs -i sudo docker rm {}

# 清理 etcd 容器（1个 etcd 容器，1个 pause 容器）
sudo docker ps -a | grep etcd | grep Exited | awk '{print $1}' | xargs -i sudo docker rm {}

# 清理 controller 容器（1个 controller 容器，1个 pause容器）
sudo docker ps -a | grep controller | grep Exited | awk '{print $1}' | xargs -i sudo docker rm {}

# 清理 scheduler 容器（1个 scheduler 容器，1个 pause 容器）
sudo docker ps -a | grep scheduler | grep Exited | awk '{print $1}' | xargs -i sudo docker rm {}
...
sudo docker ps -a | grep dashboard | grep Exited | awk '{print $1}' | xargs -i sudo docker rm {}
```
...
# 清理 mon 容器（1个 chown 容器，1个 mon 容器，1个 pause 容器）
