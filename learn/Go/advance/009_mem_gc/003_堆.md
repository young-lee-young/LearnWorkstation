...
Go 每次申请的虚拟内存单元（heapArena）为 64MB，最多可以申请 2^22 = 4194304 个 heapArena，所有的 heapArena 组成了 mheap（Go 堆内存）
...
根据隔离适应策略，使用内存时的最小单位为 mspan；mspan 是**一组**相同的小内存单元，而不是一个小内存单元
...
// 一个 heapArena 为 64 MB，一个 heapArena 有 8192 个 page，每个 page 8 KB
// mheap > heapArena > span > page > obj
// 一个 span 可能包含多个 page
...
type mcentral struct {
	// spanClass 8 位，前 7 位记录 mspan 级别，第 8 位记录是否需要扫描
...
mcentral 实际是中心索引，不能让多个协程操作 mcentral，因此需要使用互斥锁保护，在高并发场景下，锁冲突问题严重
...
每个 P（协程队列） 拥有一个 mcache（从 mcentral 获取）
...
[构建结果](images/010_逃逸分析.png)


* 分析

[内存申请、释放、逃逸分析](Go内存使用.pdf)
